...
/* ====================================================
   InitSDL : demarre SDL et ses modules
   ==================================================== */
...
/* ====================================================
   Initialisation : met tout a NULL / zero
   ==================================================== */
int Initialisation(enigme *e)
{
    ...
    /* Positions boutons reponses A B C */
...
/* ====================================================
   Load : charge images, sons et questions
   ==================================================== */
int Load(enigme *e)
{
    /* Police */
...
/* ====================================================
   NouvelleQuestion : question aleatoire non repetee
   ==================================================== */
...
/* ====================================================
   AfficherTexte : dessine du texte a l'ecran
   ==================================================== */
...
/* ====================================================
   AfficherTimeline : barre de progression coloree
   vert (15-8s) => orange (7-4s) => rouge (3-0s)
   ==================================================== */
void AfficherTimeline(enigme *e)
{
    /* largeur totale de la barre = 700 pixels, centree */
    int barreX      = 50;
    int barreY      = 50;
    int barreHauteur= 18;
    int barreMaxW   = 700;

    /* largeur actuelle selon le temps restant */
    ...
    /* vert si > 8s, orange si > 4s, rouge sinon */
    if (e->tempsRestant > 8)
        SDL_SetRenderDrawColor(e->renderer, 0, 200, 0, 255);      /* vert   */
    else if (e->tempsRestant > 4)
        SDL_SetRenderDrawColor(e->renderer, 255, 140, 0, 255);    /* orange */
    else
        SDL_SetRenderDrawColor(e->renderer, 220, 0, 0, 255);      /* rouge  */
...
/* ====================================================
   Affichage : dessine tout selon l'etat
   ==================================================== */
void Affichage(enigme *e)
{
    ...
    if (e->etat == 1)
    {
        ...
        /* ===== CAS 1 : question en cours (pas encore finie) ===== */
        if (e->enigmeFinie == 0)
        {
        ...
        /* ===== CAS 2 : question finie => afficher seulement le resultat sur bg1 ===== */
        if (e->enigmeFinie != 0)
        {
            /* fond menu (bg1) uniquement */
            if (e->bg1) SDL_RenderCopy(e->renderer, e->bg1, NULL, NULL);
            ...
            if (e->feedback == 1)
            {
                AfficherTexte(e, "WELL DONE !", 200, 200, vert, 100);
...
/* ====================================================
   GererEvenement : clics souris et clavier
   ==================================================== */
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT)
        {
            ...
            if (e->etat == 0)
            {
                ...
                {
                    if (e->beep) Mix_PlayChannel(-1, e->beep, 0);
                    /* Masquer la fenetre quiz, lancer le puzzle, puis la reafficher */
                    SDL_HideWindow(e->window);
                    RunPuzzle();
                    SDL_ShowWindow(e->window);
            ...
            if (e->etat == 1)
            {
                /* si la question est finie => clic n'importe ou = retour menu */
                if (e->enigmeFinie != 0)
                {
                    Mix_HaltMusic();
                    e->etat     = 0;
                ...
                {
                    /* question encore active : on attend le clic sur A B ou C */
                    int rep = 0;
...
/* ====================================================
   MiseAJour : gere le timer
   ==================================================== */
void MiseAJour(enigme *e)
{
    if (e->etat != 1) return;
    if (e->enigmeFinie != 0) return;  /* si finie on attend le clic */
    ...
    {
        e->tempsRestant = 0;
        e->enigmeFinie  = 2;   /* 2 = temps ecoule */
        e->score        = 0;
        if (e->laugh) Mix_PlayChannel(-1, e->laugh, 0);
    }
}
